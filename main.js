...
if (document.querySelector(".gallery-container")) {
  ...
  galleryButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      let activeContainer = Array.from(galleryContainer).find(
...
if (document.querySelector(".menu-container")) {
  let menuButtons = document.querySelectorAll(".gallery-buttons button");
  ...
  menuButtons.forEach((button) => {
    button.addEventListener("click", () => {
      ...
      let activeContainer = Array.from(menuContainer).find(
        (container) => container.getAttribute("data-category") === category
      );
      activeContainer.classList.remove("d-none");
    });
...
if (document.querySelector(".food-buttons")) {
  let categoryButtons = document.querySelectorAll(".food-buttons button");
  let food = document.querySelectorAll(".product-col");
    ...
    button.addEventListener("click", function () {
      ...
      if (categoryID === "0") {
        for (let i = 0; i < food.length; i++) {
          food[i].classList.remove("d-none");
        }
      }
      else {
        let filteredFood = Array.from(food).filter(
          (food) => food.getAttribute("data-category-id") === categoryID
        );

        for (let i = 0; i < food.length; i++) {
          food[i].classList.add("d-none");
        }
        for (let i = 0; i < filteredFood.length; i++) {
          filteredFood[i].classList.remove("d-none");
        }
      }
    });
  });
